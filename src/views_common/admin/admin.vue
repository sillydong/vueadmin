  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table :data="table.items" v-loading.body="table.loading" border fit highlight-current-row>
        ...
        <el-table-column prop="admin_name" align="left" label="昵称" width="100"></el-table-column>
        <el-table-column prop="admin_account" align="left" label="登陆账号" width="100"></el-table-column>
        <el-table-column align="left" label="权限">
          <template slot-scope="scope">
            <span>{{scope.row.auth | joinfield('description')}}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" label="最后一次登陆">
          <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="form.visible" width="800px" :title="form.title">
      <el-form ref="form" :model="form.form" label-width="100px" label-position="left"
               :rules="form.rules">
        <el-form-item label="登陆账号" prop="admin_account">
          <el-input v-model="form.form.admin_account" placeholder="登陆账号用于登陆时输入"
                    :disabled="form.action=='update'"></el-input>
        </el-form-item>
        <el-form-item label="昵称" prop="admin_name">
          <el-input v-model="form.form.admin_name" placeholder="请输入昵称"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="admin_password_a">
          <el-input v-model="form.form.admin_password_a" type="password"
                    placeholder="请输入8位以上密码"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="admin_password_b">
          <el-input v-model="form.form.admin_password_b" type="password" placeholder="请再次输入密码"></el-input>
        </el-form-item>
        <el-form-item label="角色" prop="auth">
          <el-select v-model="form.form.auth" multiple style="width: 100%">
            <el-option v-for="item in roles" :key="item.name" :value="item.name" :label="item.description"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <template>
            <el-switch v-model.number="form.form.status" :active-value="1" :inactive-value="0" active-text="启用" inactive-text="禁用"></el-switch>
          </template>
        </el-form-item>
      </el-form>
