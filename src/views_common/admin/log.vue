  ...
  <div class="app-container">
    <div class="table-container">
      <el-table :data="table.items" v-loading.body="table.loading" border fit highlight-current-row>
        ...
        <el-table-column align="left" label="操作人员" width="120">
          <template slot-scope="scope">
            <span>{{admins[scope.row.admin_id]}}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" label="IP/路径" width="200">
          <template slot-scope="scope">
            <span>{{scope.row.ip}}<br>{{scope.row.route}}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" label="操作记录">
          <template slot-scope="scope">
            <span v-html="scope.row.description"></span>
          </template>
        </el-table-column>
        <el-table-column align="left" label="操作时间" width="180">
          <template slot-scope="scope">
